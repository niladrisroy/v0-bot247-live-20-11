...
import { Menu, X, User, ArrowLeft, LogOut } from "lucide-react" // Removed ExternalLink import
                    ...
                    ? navItems.map((item) => (
                        <div key={item.name} className="relative">
                          {item.openInNewTab ? (
                            ...
                            >
                              {item.name}
                              {/* Removed ExternalLink for Docs button */}
                            </button>
                          ) : (
                            <Link
                              href={item.href}
                    ...
                    : publicNavItems.map((item) => (
                        <div key={item.name} className="relative">
                          {item.openInNewTab ? (
                            <button
                              onClick={(e) => handleNavClick(e, item.href, item.openInNewTab)}
                              className="px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-300 dark:hover:text-blue-400 flex items-center gap-1"
                            >
                              {item.name}
                              {/* Removed ExternalLink for Docs button */}
                            </button>
